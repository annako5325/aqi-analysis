...
# -*- coding: utf-8 -*-
"""
使用邊界清理後的數據創建 Folium 地圖
視覺化 AQI 測站和避難收容處所
Layer A: AQI Stations (三色分類: 0-50綠色, 51-100黃色, 101+紅色)
Layer B: Evacuation Shelters (Distinct icons for Indoor vs. Outdoor) - 已清理異常點
"""
...
class CleanedAQIShelterMap:
    """使用清理後數據的 AQI 和避難收容處所地圖生成器"""
    ...
    def add_statistics_panel(self):
        ...
        <h4 style="margin: 0 0 10px 0; color: #333;">📊 統計資訊 (已清理異常點)</h4>
    ...
    def save_map(self, filename='cleaned_aqi_shelter_map.html'):
        """儲存地圖"""
    ...
    def create_map(self, use_sample_aqi=True):
        """創建完整地圖"""
        print("=" * 60)
        print("創建邊界清理後的 AQI 和避難收容處所地圖")
        print("AQI 三色分類: 0-50綠色, 51-100黃色, 101+紅色")
        print("避難收容處所: 已移除界線外異常點位")
        print("=" * 60)
        ...
        print("邊界清理後地圖創建完成！")
        print("=" * 60)
        print("📁 輸出檔案: cleaned_aqi_shelter_map.html")
        print("🌐 請用瀏覽器開啟檔案查看地圖")
        print("🎯 AQI 三色分類:")
        print("   🟢 0-50: 良好 (綠色)")
        print("   🟡 51-100: 普通 (黃色)")
...
def main():
    """主程式"""
    map_creator = CleanedAQIShelterMap()
